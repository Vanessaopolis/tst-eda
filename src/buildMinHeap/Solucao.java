...
class Heap {
	...
	private void heapify(int indice) {
		...
		int indiceMin = indiceMin(indice, esquerdo(indice), direito(indice));

		if (indiceMin != indice) {
			troca(indice, indiceMin);
			heapify(indiceMin);
		}
	}

	private int indiceMin(int indice, int esquerdo, int direito) {
		int indiceMin = indice;

		if (ehIndiceValido(esquerdo) && this.heap[esquerdo] < this.heap[indiceMin])
			indiceMin = esquerdo;

		if (ehIndiceValido(direito) && this.heap[direito] < this.heap[indiceMin])
			indiceMin = direito;
